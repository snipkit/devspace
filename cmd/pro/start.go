...
func NewStartCmd(flags *proflags.GlobalFlags) *cobra.Command {
	cmd := &StartCmd{GlobalFlags: *flags,
		Product:   "devspace-pro",
		ChartName: "devspace-pro",
		Log:       log.Default,
	}
	...
	startCmd.Flags().StringVar(&cmd.Namespace, "namespace", "devspace-pro", "The namespace to install into")
...
Password: `+ansi.Color(password, "green+b")+`  # Change via UI or via: `+ansi.Color("devspace pro reset password", "green+b")+`
...
func (cmd *StartCmd) runInDocker(ctx context.Context, name string) (string, error) {
	...
	if cmd.DockerImage != "" {
		...
		args = append(args, "ghcr.io/khulnasoft-lab/devspace-pro:"+strings.TrimPrefix(cmd.Version, "v"))
	} else {
		args = append(args, "ghcr.io/khulnasoft-lab/devspace-pro:latest")
	}
...
func isHostReachable(ctx context.Context, host string) (bool, error) {
	...
	if err == nil && resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			return false, fmt.Errorf("error decoding response from %s: %w. Try running '%s --reset'", url, err, "devspace pro start")
		} else if v.Version == "" {
			return false, fmt.Errorf("unexpected response from %s: %s. Try running '%s --reset'", url, string(out), "devspace pro start")
		}
