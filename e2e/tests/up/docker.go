	...
	ginkgo.Context("testing up command", ginkgo.Label("up-docker"), ginkgo.Ordered, func() {
		...
		ginkgo.Context("with docker", ginkgo.Ordered, func() {
			...
			ginkgo.It("should start a new workspace with existing running container", func(ctx context.Context) {
				...
				err = dockerHelper.Run(ctx, []string{"run", "-d", "--label", "devspace-e2e-test-container=true", "-w", "/workspaces/e2e", "mcr.microsoft.com/vscode/devcontainers/base:alpine", "sleep", "infinity"}, nil, nil, nil)
				framework.ExpectNoError(err)

				ids, err := dockerHelper.FindContainer(ctx, []string{
					"devspace-e2e-test-container=true",
				})
				framework.ExpectNoError(err)
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--source", fmt.Sprintf("container:%s", containerDetail.ID))
				framework.ExpectNoError(err)
			}, ginkgo.SpecTimeout(framework.GetTimeout()))
			...
			ginkgo.It("should start a new workspace with dotfiles - no install script", func(ctx context.Context) {
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--dotfiles", "https://github.com/khulnasoft-lab/example-dotfiles")
				framework.ExpectNoError(err)
...
`
			...
			ginkgo.It("should start a new workspace with dotfiles - install script", func(ctx context.Context) {
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--dotfiles", "https://github.com/khulnasoft-lab/example-dotfiles", "--dotfiles-script", "install-example")
				framework.ExpectNoError(err)
			...
			ginkgo.It("should start a new workspace with dotfiles - no install script, commit", func(ctx context.Context) {
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--dotfiles", "https://github.com/khulnasoft-lab/example-dotfiles@sha256:9a0b41808bf8f50e9871b3b5c9280fe22bf46a04")
				framework.ExpectNoError(err)
...
`
			...
			ginkgo.It("should start a new workspace with dotfiles - no install script, branch", func(ctx context.Context) {
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--dotfiles", "https://github.com/khulnasoft-lab/example-dotfiles@do-not-delete")
				framework.ExpectNoError(err)
			...
			ginkgo.It("should start a new workspace with custom image", func(ctx context.Context) {
				...
				// Wait for devspace workspace to come online (deadline: 30s)
				err = f.DevSpaceUp(ctx, tempDir, "--devcontainer-image", "mcr.microsoft.com/vscode/devcontainers/base:alpine")
				framework.ExpectNoError(err)

				out, err := f.DevSpaceSSH(ctx, tempDir, "grep ^ID= /etc/os-release")
				framework.ExpectNoError(err)
