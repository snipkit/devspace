...
import (
	...
	"dev.khulnasoft.com/pkg/devcontainer/build"
...
var _ = DevSpaceDescribe("devspace build test suite", func() {
	ginkgo.Context("testing build", ginkgo.Label("build"), ginkgo.Ordered, func() {
		...
		ginkgo.It("build docker buildx", func() {
			...
			// do the build
			err = f.DevSpaceBuild(ctx, tempDir, "--force-build", "--platform", "linux/amd64,linux/arm64", "--repository", prebuildRepo, "--skip-push")
			framework.ExpectNoError(err)
		...
		ginkgo.It("should build image without repository specified if skip-push flag is set", func() {
			...
			// do the build
			err = f.DevSpaceBuild(ctx, tempDir, "--skip-push")
			framework.ExpectNoError(err)
		...
		ginkgo.It("should build the image of the referenced service from the docker compose file", func() {
			...
			// do the build
			err = f.DevSpaceBuild(ctx, tempDir, "--repository", prebuildRepo, "--skip-push")
			framework.ExpectNoError(err)
		})
		...
		ginkgo.It("build docker internal buildkit", func() {
			...
			// do the build
			err = f.DevSpaceBuild(ctx, tempDir, "--force-build", "--force-internal-buildkit", "--repository", prebuildRepo, "--skip-push")
			framework.ExpectNoError(err)
		...
		ginkgo.It("build kubernetes dockerless", func() {
			...
			err = f.DevSpaceProviderAdd(ctx, "kubernetes")
			framework.ExpectNoError(err)
			err = f.DevSpaceProviderUse(context.Background(), "kubernetes", "-o", "KUBERNETES_NAMESPACE=devspace")
			framework.ExpectNoError(err)
			...
			// check if ssh works
			out, err := f.DevSpaceSSH(ctx, tempDir, "echo -n $MY_TEST")
			framework.ExpectNoError(err)
		...
		ginkgo.It("rebuild kubernetes dockerless", func() {
			...
			_, err = f.DevSpaceSSH(ctx, tempDir, "touch /workspaces/"+filepath.Base(tempDir)+"/DATA")
			framework.ExpectNoError(err)
			_, err = f.DevSpaceSSH(ctx, tempDir, "touch /ROOTFS")
			framework.ExpectNoError(err)
			...
			_, err = f.DevSpaceSSH(ctx, tempDir, "ls /workspaces/"+filepath.Base(tempDir)+"/DATA")
			framework.ExpectNoError(err)
			// this should fail! because --recreare should trigger a new build, so a new rootfs
			_, err = f.DevSpaceSSH(ctx, tempDir, "ls /ROOTFS")
		...
		ginkgo.It("reset kubernetes dockerless", func() {
			...
			// recreate
			err = f.DevSpaceUpReset(ctx, tempDir)
			framework.ExpectNoError(err)
