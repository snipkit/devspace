...
func (o *GenericJetBrainsServer) OpenGateway(workspaceFolder, workspaceID string) error {
	...
	err := open.Run(`jetbrains-gateway://connect#idePath=` + url.QueryEscape(o.getDirectory(path.Join("/", "home", o.userName))) + `&projectPath=` + url.QueryEscape(workspaceFolder) + `&host=` + workspaceID + `.devspace&port=22&user=` + url.QueryEscape(o.userName) + `&type=ssh&deploy=false`)
	if err != nil {
...
func (o *GenericJetBrainsServer) GetVolume() string {
	return fmt.Sprintf("type=volume,src=devspace-%s,dst=%s", o.options.ID, o.getDownloadFolder())
}

func (o *GenericJetBrainsServer) getDownloadFolder() string {
	return fmt.Sprintf("/var/devspace/%s", o.options.ID)
}
...
func (o *GenericJetBrainsServer) download(targetFolder string, log log.Logger) (string, error) {
	...
	resp, err := devspacehttp.GetHTTPClient().Get(downloadURL)
	if err != nil {
