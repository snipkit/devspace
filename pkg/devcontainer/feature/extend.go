...
import (
	...
	"dev.khulnasoft.com/pkg/copy"
	"dev.khulnasoft.com/pkg/devcontainer/config"
	"dev.khulnasoft.com/pkg/devcontainer/graph"
...
COPY ./` + config.DevSpaceContextFeatureFolder + `/ /tmp/build-features/
...
func getFeatureBuildOptions(contextPath string, imageBuildInfo *config.ImageBuildInfo, target string, features []*config.FeatureSet) (*BuildInfo, error) {
	...
	// copy features
	featureFolder := filepath.Join(contextPath, config.DevSpaceContextFeatureFolder)
