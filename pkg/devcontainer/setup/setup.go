...
import (
	...
	copy2 "dev.khulnasoft.com/pkg/copy"
...
const (
	ResultLocation = "/var/run/devspace/result.json"
)

func SetupContainer(ctx context.Context, setupInfo *config.Result, extraWorkspaceEnv []string, chownProjects bool, platformOptions *devspace.PlatformOptions, tunnelClient tunnel.TunnelClient, log log.Logger) error {
...
func markerFileExists(markerName string, markerContent string) (bool, error) {
	markerName = filepath.Join("/var/devspace", markerName+".marker")
...
func setupPlatformGitCredentials(userName string, platformOptions *devspace.PlatformOptions, log log.Logger) error {
...
func setupPlatformGitHTTPCredentials(userName string, platformOptions *devspace.PlatformOptions, log log.Logger) error {
...
func setupPlatformGitSSHKeys(userName string, platformOptions *devspace.PlatformOptions, log log.Logger) error {
