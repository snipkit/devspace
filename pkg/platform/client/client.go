...
func (c *client) restConfig(hostSuffix string) (*rest.Config, error) {
	if c.config == nil {
		...
		return nil, perrors.New(fmt.Sprintf("not logged in, please make sure you have run '%s' to create one or '%s [%s]' if one already exists", "devspace pro start", "devspace pro login", "devspace-pro-url"))
	}
...
func GetRestConfig(host, token string, insecure bool) (*rest.Config, error) {
	...
	config.UserAgent = "devspace/" + version.GetVersion()
