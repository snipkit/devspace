...
func (d *dockerDriver) RunDockerDevContainer(
	...
	if runtime.GOOS == "linux" && ((parsedConfig.ContainerUser != "" || parsedConfig.RemoteUser != "") &&
		...
		containerPasswdFileIn, err := os.CreateTemp("", "devspace_container_passwd_in")
		if err != nil {
			return err
		}
		defer os.Remove(containerPasswdFileIn.Name())

		containerGroupFileIn, err := os.CreateTemp("", "devspace_container_group_in")
		if err != nil {
			return err
		}
		defer os.Remove(containerGroupFileIn.Name())

		containerPasswdFileOut, err := os.CreateTemp("", "devspace_container_passwd_out")
		if err != nil {
			return err
		}
		defer os.Remove(containerPasswdFileOut.Name())

		containerGroupFileOut, err := os.CreateTemp("", "devspace_container_group_out")
		if err != nil {
			return err
		}
