...
const DevContainerName = "devspace"
...
func getVolumeMount(idx int, mount *config.Mount) corev1.VolumeMount {
	...
	return corev1.VolumeMount{
		...
		SubPath:   fmt.Sprintf("devspace/%s", subPath),
	}
}
...
func getID(workspaceID string) string {
	return "devspace-" + workspaceID
}

func getPullSecretsName(workspaceID string) string {
	return fmt.Sprintf("devspace-pull-secret-%s", workspaceID)
}

func getDaemonSecretName(workspaceID string) string {
	return fmt.Sprintf("devspace-daemon-secret-%s", workspaceID)
}
