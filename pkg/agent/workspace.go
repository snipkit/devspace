...
var extraSearchLocations = []string{"/home/devspace/.devspace/agent", "/opt/devspace/agent", "/var/lib/devspace/agent", "/var/devspace/agent"}

var ErrFindAgentHomeFolder = fmt.Errorf("couldn't find devspace home directory")
...
func findDir(agentFolder string, validate func(path string) bool) string {
	...
	if homeDir != "" {
		homeDir = filepath.Join(homeDir, ".devspace", "agent")
		if validate(homeDir) {
			return homeDir
		}
	}
...
func CloneRepositoryForWorkspace(
	...
	if platformGitcacheEnabled {
		...
		// marshal options
		jsonOptions, err := json.Marshal(&devspace.CloneOptions{
		...
		_, err = devspace.NewRunnerClient(grpcClient).Clone(ctx, &devspace.CloneRequest{
	...
	// Get .devspaceignore files to exclude
	f, err := os.Open(filepath.Join(workspaceDir, ".devspaceignore"))
	if err != nil {
		return nil
	}
	excludes, err := ignorefile.ReadAll(f)
	if err != nil {
		log.Warn(".devspaceignore file is invalid : ", err)
		return nil
	}
	...
	log.Debug("Ignore files from .devspaceignore ", excludes)
