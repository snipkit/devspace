...
func InjectAgentAndExecute(
	...
	// install devspace into the target
	...
	for {
		...
		if err != nil {
			...
			if time.Since(lastMessage) > time.Second*5 {
				log.Infof("Waiting for devspace agent to come up...")
				lastMessage = time.Now()
			}
...
func downloadAgentLocally(tryDownloadURL, targetArch string, log log.Logger) (string, error) {
	agentPath := filepath.Join(os.TempDir(), "devspace-cache", "devspace-linux-"+targetArch)
	...
	if err != nil {
		return "", fmt.Errorf("download devspace: %w", err)
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("failed to download devspace from URL %s: %w", fullDownloadURL, err)
	}
...
func getRunnerBinary(targetArch string) string {
	binaryPath := filepath.Join(os.TempDir(), "devspace-cache", "devspace-linux-"+targetArch)
	_, err := os.Stat(binaryPath)
	if err != nil {
		return ""
	}
