...
func NewMachineClient(devSpaceConfig *config.Config, provider *provider.ProviderConfig, machine *provider.Machine, log log.Logger) (client.MachineClient, error) {
...
func (s *machineClient) AgentPath() string {
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, nil, s.machine).Path
}

func (s *machineClient) AgentLocal() bool {
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, nil, s.machine).Local == "true"
}

func (s *machineClient) AgentURL() string {
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, nil, s.machine).DownloadURL
}
...
func runCommand(ctx context.Context, name string, command types.StrArray, environ []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, log log.Logger) (err error) {
	...
	if log.GetLevel() == logrus.DebugLevel {
		environ = append(environ, DevSpaceDebug+"=true")
	}
...
func printStillRunningLogMessagePeriodically(log log.Logger) chan struct{} {
	return printLogMessagePeriodically("Please hang on, DevSpace is still running, this might take a while...", log)
}
