...
import (
	...
	sshServer "dev.khulnasoft.com/pkg/ssh/server"
	"dev.khulnasoft.com/pkg/ts"
...
func (c *client) CheckWorkspaceReachable(ctx context.Context) error {
	...
	if err != nil {
		...
		if daemon.IsDaemonNotAvailableError(getWorkspaceErr) {
			deeplink := fmt.Sprintf("devspace://open?workspace=%s&provider=%s&source=%s&ide=%s", c.workspace.ID, c.config.Name, c.workspace.Source.String(), c.workspace.IDE.Name)
		...
		if getWorkspaceErr != nil {
			...
			return fmt.Errorf("workspace is '%s', please run `devspace up %s` to start it again", instance.Status.Phase, c.workspace.ID)
		} else if instance.Status.LastWorkspaceStatus != storagev1.WorkspaceStatusRunning {
			return fmt.Errorf("workspace is '%s', please run `devspace up %s` to start it again", instance.Status.LastWorkspaceStatus, c.workspace.ID)
		}
