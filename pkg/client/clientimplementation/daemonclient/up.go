...
import (
	...
	"dev.khulnasoft.com/pkg/platform/kube"
	"dev.khulnasoft.com/log"
	corev1 "k8s.io/api/core/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"

	_ "dev.khulnasoft.com/api/pkg/apis/management/install" // Install the management group to ensure the option types are registered
)
...
func printLogs(ctx context.Context, managementClient kube.Interface, workspace *managementv1.DevSpaceWorkspaceInstance, taskID string, logger log.Logger) (int, error) {
	...
	stdoutStreamer, stdoutDone := devspacelog.PipeJSONStream(logger)
	stderrStreamer, stderrDone := devspacelog.PipeJSONStream(logger.ErrorStreamOnly())
	defer func() {
		// close the streams
