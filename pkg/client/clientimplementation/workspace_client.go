...
func NewWorkspaceClient(devSpaceConfig *config.Config, prov *provider.ProviderConfig, workspace *provider.Workspace, machine *provider.Machine, log log.Logger) (client.WorkspaceClient, error) {
...
func (s *workspaceClient) AgentLocal() bool {
	...
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, s.workspace, s.machine).Local == "true"
}
...
func (s *workspaceClient) AgentPath() string {
	...
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, s.workspace, s.machine).Path
}
...
func (s *workspaceClient) AgentURL() string {
	...
	return options.ResolveAgentConfig(s.devSpaceConfig, s.config, s.workspace, s.machine).DownloadURL
}
...
func (s *workspaceClient) RefreshOptions(ctx context.Context, userOptionsRaw []string, reconfigure bool) error {
	...
	if s.isMachineProvider() {
		...
		machine, err := options.ResolveAndSaveOptionsMachine(ctx, s.devSpaceConfig, s.config, s.machine, userOptions, s.log)
		if err != nil {
			return err
		}
	...
	workspace, err := options.ResolveAndSaveOptionsWorkspace(ctx, s.devSpaceConfig, s.config, s.workspace, userOptions, s.log)
	if err != nil {
		return err
	}
...
func (s *workspaceClient) agentInfo(cliOptions provider.CLIOptions) *provider.AgentWorkspaceInfo {
	...
	agentInfo := &provider.AgentWorkspaceInfo{
		...
		Agent:                  options.ResolveAgentConfig(s.devSpaceConfig, s.config, s.workspace, s.machine),
		Options:                s.devSpaceConfig.ProviderOptions(s.Provider()),
	}
	...
	agentInfo.InjectTimeout = config.ParseTimeOption(s.devSpaceConfig, config.ContextOptionAgentInjectTimeout)

	// Set registry cache from context option
	agentInfo.RegistryCache = s.devSpaceConfig.ContextOption(config.ContextOptionRegistryCache)
...
func (s *workspaceClient) Create(ctx context.Context, options client.CreateOptions) error {
	...
	machineClient, err := NewMachineClient(s.devSpaceConfig, s.config, s.machine, s.log)
	if err != nil {
		return err
	}
...
func (s *workspaceClient) Command(ctx context.Context, commandOptions client.CommandOptions) (err error) {
	...
	s.m.Lock()
	environ, err := binaries.ToEnvironmentWithBinaries(s.workspace.Context, s.workspace, s.machine, s.devSpaceConfig.ProviderOptions(s.config.Name), s.config, map[string]string{
...
func (s *workspaceClient) getContainerStatus(ctx context.Context) (client.Status, error) {
	...
	err = RunCommandWithBinaries(ctx, "command", s.config.Exec.Command, s.workspace.Context, s.workspace, s.machine, s.devSpaceConfig.ProviderOptions(s.config.Name), s.config, map[string]string{
		provider.CommandEnv: command,
