...
```sh
devspace provider update <provider-name> my-org/my-repo
```

If you want to update (or downgrade) a specific version, you can do:
```sh
devspace provider update <provider-name> github.com/my-org/my-repo@v0.2.0
```
...
```sh
devspace provider update <provider-name> http://path/to/my/provider.yaml
```
...
```sh
devspace provider update <provider-name> ../path-to-updated/provider.yaml
```
