use crate::{commands::DEVSPACE_BINARY_NAME, AppHandle};
use log::error;
use std::path::Path;
use std::str::Lines;
...
// The path to the `devspace-cli` binary/executable. If bundled correctly, will be placed next to the desktop app executable.
fn get_cli_path() -> Result<PathBuf, std::io::Error> {
    let mut exe_path = env::current_exe()?;
    exe_path.pop();
    exe_path.push(DEVSPACE_BINARY_NAME);
...
fn install(_app_handle: AppHandle, force: bool) -> Result<(), InstallCLIError> {
    ...
    // The binary we ship with is `devspace-cli`, but we want to link it to `devspace` so that users can just run `devspace` in their terminal
    let mut target_paths: Vec<PathBuf> = vec![];

    // /usr/local/bin/devspace
    let raw_system_bin = format!("/usr/local/bin/{}", "devspace");
...
fn install(app_handle: AppHandle, force: bool) -> Result<(), InstallCLIError> {
    ...
    let cmd_file = BinFile {
        name: format!("{}.cmd", "devspace".to_string()),
