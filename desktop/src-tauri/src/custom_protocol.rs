...
mod tests {
    mod url_parser {
        ...
        #[test]
        fn should_parse() {
            let url_str = "devspace://open?workspace=workspace";
        ...
        fn should_fail_on_invalid_method() {
            let url_str = "devspace://something";
    ...
    mod custom_handler_open {
        ...
        fn should_parse_full() {
            let url_str =
                "devspace://open?workspace=workspace&provider=provider&source=https://github.com/test123&ide=vscode";
            let request = UrlParser::parse(&url_str).unwrap();
        ...
        fn should_parse_workspace() {
            let url_str = "devspace://open?workspace=some-workspace";
            let request = UrlParser::parse(&url_str).unwrap();
            let got: OpenWorkspaceMsg = CustomProtocol::parse(&request).unwrap();
        ...
        #[test]
        fn should_parse() {
            let url_str = "devspace://open?source=some-source";
            let request = UrlParser::parse(&url_str).unwrap();
            let got: OpenWorkspaceMsg = CustomProtocol::parse(&request).unwrap();
    ...
    mod custom_handler_import {
        ...
        fn should_parse_full() {
            let url_str =
                "devspace://import?workspace-id=workspace&workspace-uid=uid&devspace-pro-host=devspace.pro&other=other&project=foo";
            let request = UrlParser::parse(&url_str).unwrap();
            ...
            assert_eq!(got.devspace_pro_host, "devspace.pro".to_string());
        ...
        fn should_fail_on_missing_workspace_id() {
            let url_str =
                "devspace://import?workspace-uid=uid&devspace-pro-host=devspace.pro&other=other";
            let request = UrlParser::parse(&url_str).unwrap();
    ...
    mod custom_handler_pro_setup {
        ...
        #[test]
        fn should_parse_full() {
            let url_str = "devspace://pro/setup?host=foo&access_key=bar";
            let request = UrlParser::parse(&url_str).unwrap();
        ...
        fn should_parse_host() {
            let url_str = "devspace://pro/setup?host=localhost%3A8080";
            let request = UrlParser::parse(&url_str).unwrap();
